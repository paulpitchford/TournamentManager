...
namespace TournamentManager.Infrastructure.Validators
{
    public class PointStructureValidator : AbstractValidator<PointStructure>
    {
        ...
        public PointStructureValidator(IPoints points)
        {
            ...
            RuleFor(ps => ps.PointPositions).Must(pp => pp.Max(p => p.Position) + 1 == _points.NextPosition(pp.Select(p => p.Position).ToList())).When(pp => pp.PointPositions.Count > 0).WithMessage("The list of positions does not run equal. Pleas ensure they run equal e.g. 1-10.");
        }
    }
}
