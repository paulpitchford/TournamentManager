...
namespace TournamentManager.Web.Pages.Settings.Seasons
{
    public partial class Edit
    {
        ...
        {
            formLoading = true;
            Season = await _apiClient.httpClient.GetFromJsonAsync<Season>($"/api/Season/{Id}");
            formLoading = false;
        }
        ...
        async Task OnSubmit()
        {
            if (Season != null)
            {
                try
                {
                    var response = await _apiClient.httpClient.PutAsJsonAsync<Season>($"/api/Season/{Id}", Season);
